...
**Architecture (x86, arm, RISC, 32-bit, 64-bit)** This refers to core design of the CPU (or System-on-Chip) and it's methods of communications (instruction set). Code is not interoperable between architectures, with rare exceptions. 'x86' and 'arm' are the most common architectures. Traditional 'x86' processors are common in desktop and laptop computers. 'arm' (Advanced RISC Machine) are low on energy consumption, suitable for mobile. 32-bit architectures are legacy platforms that use 32 bits (rather than 64) to store data (binary integers).
